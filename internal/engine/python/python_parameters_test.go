...
func assertParameters(t *testing.T, src string, function string, expected ...string) {
	...
	file, err := engine.CreateTestFileWithCode(&PythonRunner{}, src)
	if err != nil {
...
func Test_Annotations_Are_Not_Parameters(t *testing.T) {
	assertParameters(t, `
class Repository:
    def find(self, identifier: int, label: str = "none"):
        pass
`, "find", "self", "identifier", "label")
}

func Test_Stars_And_Separators_Are_Not_Parameters(t *testing.T) {
	assertParameters(t, `
def build(a, /, b, *args, c: int = 1, **kwargs):
    pass
`, "build", "a", "b", "args", "c", "kwargs")
}
